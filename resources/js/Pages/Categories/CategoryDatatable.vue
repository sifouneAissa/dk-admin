...
export default {
    ...
    methods: {
        editCategory(data) {
            console.log(data);
            this.$inertia.visit(route('admin.category.edit', {
                category : data.id
            }));
        },
        loadLazyData() {
            ...
            setTimeout(() => {
                CategoryService.getCategories({ lazyEvent: JSON.stringify(this.lazyParams) }).then((data) => {
        ...
        openNew() {
            this.$inertia.visit(route('admin.category.create'));
        },
        confirmDeleteProduct(data) {
            this.confirm$.require({
                ...
                accept: () => {
                    this.$inertia.delete(route('admin.category.destroy',{
                        category : data.id
                    }),{
                        onSuccess : () => {
